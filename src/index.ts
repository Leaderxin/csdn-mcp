...
// 生成 HMAC 签名
function generateSignature(method: string, uri: string, accept: string, contentType: string, nonce: string): string {
...
async function csdnFetch(path: string, body: Record<string, any>): Promise<CsdnResponse> {
  ...
  const signature = generateSignature("POST", path, accept, contentType, nonce);
...
server.tool(
  ...
  async ({ cookie }) => {
    ...
    try {
      ...
      const accept = "*/*";
      const contentType = "";
      const signature = generateSignature("GET", uri, accept, contentType, nonce);
