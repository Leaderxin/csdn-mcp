...
对抗式复核（变异测试 + 探针）在**已发布的 v1.0.0** 上发现的问题。方法是「把不变量改坏，看有没有测试抓到」：25 个变异里 6 个存活，每一个存活都指向一条**没有被任何测试保护**的断言。

### Fixed

- **重试会写出重复文章（高危）**。`saveArticle` 没有幂等键，而客户端默认会对 `POST` 重试。一次 502、或一次客户端超时之后，同一个请求体会被原样再发一遍，于是创建出**第二篇文章**——正是 `KNOWN-ISSUES.md` B1 描述的那类不可自行回收的后果。变异测试证实这个行为没人钉住：把保存的重试次数改成 0 或 2，整套测试依然全绿。
  现在 `RequestOptions.idempotent` 默认「GET 为 true，其余为 false」，**非幂等请求只在服务端明确拒绝（`RATE_LIMITED`）时重试**——那是唯一能确定「什么都没写进去」的失败。为吸收 CSDN 限流而做的重试因此保留了下来。
- **重试绕过了写间隔**。限流器原先在重试循环外只取一次，于是一次保存能在 1.5 秒内连打 CSDN 三次，而它要求的是 10 秒。现在每次尝试都经过限流器。
- **失败被读成成功**。`unwrapEnvelope` 用 `code ?? 200` 兜底，导致 `{}`、`{"status":500,...}` 这类**没有 `code`** 的响应体一律按成功处理。CSDN 的 phoenix 错误用的是 `status` 而不是 `code`，所以一次失败的删除会报告自己已经删掉了。现在缺少 `code` 时，只有「带 `data` 且不含任何错误信号」才被接受，其余抛 `MALFORMED_RESPONSE`。
- **对象存储可能收到 Cookie 和签名**。注释写着 `formData` 隐含 `signed: false`，代码实际是 `?? true`。今天唯一的调用方显式传了 flag，因此没有真实泄漏，但下一个相信注释的调用方会把 CSDN cookie 发给华为 OBS 的地址。现在注释与代码一致（由 `formData` 推导）。
- **签名辅助字段被当请求头发出去**。`buildSignHeaders` 返回的 `nonce` / `uri` 只是内部字段，却被整个对象 spread 进了请求头，每次签名请求都多带两个头。
- **脱敏只按字段名白名单，不看值**。真实 cookie 里的 `c_session_id`、`Hm_up_...`、`bt_user_priv_var`、`log_Id_...` 都带可变后缀，只匹配家族名会把整条 cookie 原样留下。现在家族名容忍后缀。此外两个脱敏调用点此前**可以被删掉而测试全绿**（唯一的「无泄漏」断言用的夹具恰好在白名单上），已补测试钉住。

### Changed

- `npm run test:live` 原先指向 `tests/live/**`，而该目录不存在，脚本匹配不到任何文件。现在它先构建、再运行 `scripts/live-smoke.mjs`（该脚本自身要求 `CSDN_LIVE=1` 才肯启动）。已删除无用的 `vitest.live.config.ts`。
- `docs/ARCHITECTURE.md` §5 描述了并不存在的 `createTestContext(...)`、`tests/fixtures/` 和 `tests/live/**`，已按实际结构改写。

### Security

- secret-scan 加 `-i`：`usertoken=` 这种小写拼写此前能完整绕过扫描。
- secret-scan 的 `.env` 规则扩展到 `.env.local` / `.env.production` 等（`.env.example` 仍按设计跟踪）。
- 本次为脱敏新增的测试夹具一开始用了 32 位 hex 值，**被自己的扫描规则抓到**——已换成明显非真实的值。这正是该 job 注释里「扫描器一旦对自己的测试数据喊狼来了，就会被关掉」要防的情况。

### Known gaps

- 脱敏仍是**按字段名**匹配，不是按值清洗：值单独出现（字段名对不上）、被 URL 编码或被 base64 之后仍然会漏。今天没有任何调用点记录 cookie 或请求头，所以这是潜在缺口而非现实泄漏；按值擦除需要把 cookie 注册进 sink。
